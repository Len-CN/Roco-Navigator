...
class Sidebar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # ---- 追踪控制 ----
        tracking_section = SidebarSection("Position Tracking")

        # 追踪状态行
        status_row = QHBoxLayout()
        self._tracking_indicator = StatusIndicator()
        self._tracking_indicator.set_status("idle")
        self._tracking_status_label = NeumorphicLabel("Idle", level="body")
        ...
        # 校准按钮
        self._calibrate_btn = NeumorphicButton("Calibrate Minimap")
        self._calibrate_btn.clicked.connect(self.calibrate_clicked.emit)
        tracking_section.add_widget(self._calibrate_btn)

        # 开始/停止追踪
        track_row = QHBoxLayout()
        self._start_track_btn = NeumorphicButton("Start", primary=True)
        self._stop_track_btn = NeumorphicButton("Stop")
        ...
        # ---- 导航控制 ----
        nav_section = SidebarSection("Navigation")

        self._plan_btn = NeumorphicButton("Plan Route")
        self._plan_btn.clicked.connect(self.plan_route_clicked.emit)
        nav_section.add_widget(self._plan_btn)

        nav_row = QHBoxLayout()
        self._start_nav_btn = NeumorphicButton("Navigate", primary=True)
        self._stop_nav_btn = NeumorphicButton("Stop")
        ...
        self._nav_info_label = NeumorphicLabel("No active route", level="caption")
        ...
        # ---- 显示控制 ----
        display_section = SidebarSection("Display")

        # 悬浮窗开关
        overlay_row = QHBoxLayout()
        overlay_label = NeumorphicLabel("Overlay HUD", level="body")
        ...
        # ---- 设置 ----
        settings_section = SidebarSection("Settings")

        self._settings_btn = NeumorphicButton("Settings")
    ...
    def set_tracking_active(self, active: bool):
        ...
        if active:
            self.set_tracking_status("tracking", "Tracking...")
        else:
            self.set_tracking_status("idle", "Idle")
